...
**Goal:** Verify that the agent correctly accesses and retrieves definitions from the `@./helpers/` directory.

### Test 1.1: Spacing Helpers
- **Prompt:** "What are the available helpers for margins and their specific pixel values?"
- **Expected Result:** The agent should list `tiny-margin` (4px), `small-margin` (8px), `margin` (16px), `medium-margin` (24px), and `large-margin` (32px), citing `@./helpers/spacing.md`.

### Test 1.2: Elevation Scale
- **Prompt:** "List all available elevation levels and their corresponding Material Design DP values."
- **Expected Result:** The agent should list the scale from `no-elevate` (0dp) to `large-elevate` (16dp) using information from `@./helpers/elevation.md`.
...
- **Expected Result:** The agent should explain that BeerCSS does not use those specific classes for card sub-sections. It should provide the correct pattern using standard HTML tags (like `<h5>` for the header and a `div` for the body) inside an `<article>` tag, as documented in `@./components/cards.md`.
...
- **Expected Result:** The agent should generate code using:
  ...
  - References to `@./components/grid.md`, `@./components/cards.md`, and `@./components/navigation.md`.
...
- **Expected Result:** The agent should provide two `<nav>` elements:
  ...
  - Correct internal structure (icons + labels) based on `@./components/main-layouts.md`.
...
- **Expected Result:** The agent should identify `var(--primary-container)` and its corresponding `on-primary-container` text color, referencing `@./rules/color-system.md`.
...
- **Expected Result:** The agent should correctly retrieve `#9ecaff` from the Dark Theme reference in `@./rules/color-system.md`.
